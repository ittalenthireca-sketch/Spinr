...
def validate_coordinates(
    latitude: Union[float, int, str], longitude: Union[float, int, str], raise_exception: bool = True
    ...
    try:
        ...
        if raise_exception:
            raise HTTPException(status_code=400, detail="Coordinates must be numeric values") from None
        return False, None
...
def validate_monetary_amount(
    ...
    try:
        ...
        if raise_exception:
            raise HTTPException(status_code=400, detail="Amount must be a valid number") from None
        return False, None
    ...
    if decimal_amount < Decimal(str(min_value)):
        if raise_exception:
            raise HTTPException(status_code=400, detail=f"Amount must be at least {min_value}")
        return False, None

    if decimal_amount > Decimal(str(max_value)):
        if raise_exception:
            raise HTTPException(status_code=400, detail=f"Amount must not exceed {max_value}")
        return False, None
...
def validate_uuid(uuid_value: str, raise_exception: bool = True) -> Tuple[bool, Optional[str]]:
    """
...
def validate_id(id_value: str, id_type: str = "ID", raise_exception: bool = True) -> Tuple[bool, Optional[str]]:
    """
...
def sanitize_string(
    ...
    if len(value) > max_length:
        if raise_exception:
            raise HTTPException(status_code=400, detail=f"String exceeds maximum length of {max_length} characters")
        return False, None
...
def validate_datetime(
    ...
    if min_date and parsed_dt < min_date:
        if raise_exception:
            raise HTTPException(status_code=400, detail=f"Date must be after {min_date.isoformat()}")
        return False, None

    if max_date and parsed_dt > max_date:
        if raise_exception:
            raise HTTPException(status_code=400, detail=f"Date must be before {max_date.isoformat()}")
        return False, None
...
def validate_address(address: str, raise_exception: bool = True) -> Tuple[bool, Optional[str]]:
    """
    ...
    if not re.search(r"[a-zA-Z0-9]", address):
        if raise_exception:
            raise HTTPException(status_code=400, detail="Address must contain alphanumeric characters")
        return False, None
...
def validate_ride_location(
    pickup_lat: Any, pickup_lng: Any, dropoff_lat: Any, dropoff_lng: Any, raise_exception: bool = True
    ...
    Raises:
        HTTPException: If raise_exception is True and validation fails
    """
    valid_pickup, pickup_coords = validate_coordinates(pickup_lat, pickup_lng, raise_exception)
    if not valid_pickup:
        return False, None

    valid_dropoff, dropoff_coords = validate_coordinates(dropoff_lat, dropoff_lng, raise_exception)
    ...
    return True, (pickup_coords[0], pickup_coords[1], dropoff_coords[0], dropoff_coords[1])
